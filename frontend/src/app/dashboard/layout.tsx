...
export default function DashboardLayout({ children }: {children: React.ReactNode }) {
  ...
  const features = [
    ...
    },
    {
      ...
      text: 'Light Mode',
      mode: (
        <span className={`${darkMode ? 'bg-[#0F1A4F]' : 'bg-[#0F1A4F]'} w-10 h-5 ml-8 rounded-full flex items-center`}>
          <span 
            onClick={toggleTheme} 
            className={`bg-linear-to-b from-[#FFFFFF] to-[#0F1A4F] w-4 h-4 rounded-full ${darkMode ? 'ml-5' : 'ml-1'} transition-all duration-300 cursor-pointer`}
          ></span>
        </span>
      )      
    },
  ...
  return (
    <div className={`flex py-10 h-[922] px-20 ${darkMode ? 'dark' : 'bg-white text-black'}`}>
        <div className={`flex flex-col py-5 ${darkMode ? 'bg-gray-800' : 'bg-[#f2f2f2]'} w-[300px] rounded-lg`}>
            ...
            <div className={`flex items-center mt-11 ${darkMode ? 'bg-gray-700' : 'bg-white'} px-2 py-3 rounded-sm`}>
              <img src={assets.profile.src} alt="" className="w-12 h-12 rounded-full"/>
              <div className="ml-3">
            ...
            <div className='mt-7 flex flex-col gap-3'>
              {features.map((feature, index) => (
                <div key={index} onClick={() => setActive(index)}>
                  <p className={`flex items-center gap-3 px-5 py-3 ${active === index ? 'text-white bg-gradient-to-b from-[#172B85] to-[#559BED] rounded-md' : `hover:${darkMode ? 'bg-gray-700' : 'bg-gray-200'} text-[16px]`}`}>
                    {feature.icon} 
                    {feature.text} 
            ...
            <div className='px-5 mt-auto'>
              ...
              <p className='flex items-center gap-5 mt-6 text-[16px] text-[#FD3838]'><svg width="19" height="14" viewBox="0 0 19 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M7 1H3C1.89543 1 1 1.89543 1 3V11C1 12.1046 1.89543 13 3 13H7" stroke="#FD3838" stroke-width="1.3" stroke-linecap="round"/>
                    <path d="M14 3L17.7472 6.8607C17.8225 6.9383 17.8225 7.0617 17.7472 7.1393L14 11" stroke="#FD3838" stroke-width="1.3" stroke-linecap="round"/>
                  </svg> Log out
              </p>
            </div>
        </div>
